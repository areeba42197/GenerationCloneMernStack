  ...
  const fetchKurtas = async () => {
    try {
      ...
      const filteredKurtas = data.products.filter((product) => {
        ...
        return product.category && product.category.slug && product.category.slug.toLowerCase() === "matching-2-piece";
      });
  ...
  return (
    <Layout>
      <div className="container mt-5">
        <h2 className="text-center mb-4">Matching 2 Piece </h2>
        <div className="row">
          {kurtas.length > 0 ? (
            kurtas.map((k) => (
            ...
            <p className="text-center">No Kurtas available.</p>
          )}
        </div>
      </div>
    </Layout>
