  ...
  const fetchKurtas = async () => {
    try {
      ...
      const filteredKurtas = data.products.filter((product) => {
        ...
        return product.category && product.category.slug && product.category.slug.toLowerCase() === "formals";
      });
  ...
  return (
    <Layout>
      <div className="container mt-5">
        <h2 className="text-center mb-4">Formals </h2>
        <div className="row">
          {kurtas.length > 0 ? (
            kurtas.map((k) => (
            ...
            <p className="text-center">No Formals available.</p>
          )}
        </div>
      </div>
    </Layout>
